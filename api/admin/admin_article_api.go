...
func NewArticleController() *ArticleController {
	return &ArticleController{service.NewArticleService()}
}
...
func (article *ArticleController) ArticleFileWay(c *gin.Context) {
	articleID := c.Param("id")
	var articleFile views.AdminArticleAddFileVo
	...
	if articleFile.Author == "" {
		...
		if curUser, exists := c.Get("userInfo"); exists {
			// 其实此key不可能不存在
			if val, ok := curUser.(models.User); ok {
				// 其实类型推导也不可能不ok
				articleFile.Author = val.Username
			}
		}
	}
	ans, err := article.ArticleADDFile(articleFile, articleID)
	if err != nil {
...
func (article *ArticleController) ArticleFormWay(c *gin.Context) {
	articleID := c.Param("id")
	var articleForm views.AdminArticleAddFormVo
	...
	ans, err := article.ArticleADDForm(articleForm, articleID)
	if err != nil {
		c.JSON(http.StatusBadRequest, msg.NewMsg().Failed(err.Error()))
		return
	}
